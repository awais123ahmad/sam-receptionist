...
import { fetchUnits } from '../../../Api/Reducers/StockReducers/measurement'
...
const Unit = () => {
  ...
  const response = useSelector((state) => state.measurement);
  ...
  useEffect(() => {
    setInitialData(response.units)
  }, [response.units])

  useEffect(() => {
    setFilteredData(initialData.filter(unit => unit.unitName?.toLowerCase().includes(search.toLowerCase())));
  }, [search, initialData]);
  ...
  const sortedData = [...filteredData]?.sort((a, b) => {
    if (sortField === "unit") {
      return sortOrder === "asc" ? a.unitName.localeCompare(b.unitName) : sortOrder === "desc" ? b.unitName.localeCompare(a.unitName) : 0;
  ...
  const deleteUnit = (id) => {
    // dispatch(DeleteUnit(id)); Here use Dispatch
    setDeleteOpen(!open);
  };
  ...
  return (
    <div>
      ...
      {response.loading ?
        ...
        <>
          {response.noData ?
            <DataNotFound title={"No Unit Found"} button={"+ Add Unit"} />
            :

            <div className="p-4">
              <ListHead
                title={"Unit Table"}
                records={response.units.length}
                buttonText={"+ Add Unit"}
                searchData={search}
                    ...
                    <tr className=" text-[#101418] capitalize leading-normal w-[100%] ">
                      ...
                      <th className="py-[1%] w-[20%] text-[.8rem] text-gray-700 text-left">
                        ...
                        <Tooltip title={`Change Order`} placement="bottom">
                          <span onClick={() => toggleSortOrder('unit')} className="ml-2 cursor-pointer">
                    ...
                    <tbody key={index} className="text-gray-600 text-sm font-light w-[100%] border-t-[1px] border-gray-200">
                      <tr className={` bg-white  `}>
                        ...
                        <td className=" w-[20%] max-md:text-[.7rem] text-left">
                          <p className="font-[600] text-gray-600 text-[14px]">{value.unitName}</p>
                        </td>
