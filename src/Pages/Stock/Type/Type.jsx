...
import { fetchTypes } from '../../../Api/Reducers/StockReducers/type'
...
const Type = () => {
  ...
  // <-------------------------Fetching response-------------------------->
  const response = useSelector((state) => state.type);
  ...
  useEffect(() => {
    setInitialData(response.types)
  }, [response.types])

  useEffect(() => {
    setFilteredData(initialData.filter(type => type.typeName?.toLowerCase().includes(search.toLowerCase())));
  }, [search, initialData]);
  ...
  const sortedData = [...filteredData]?.sort((a, b) => {
    if (sortField === "type") {
      return sortOrder === "asc" ? a.typeName.localeCompare(b.typeName) : sortOrder === "desc" ? b.typeName.localeCompare(a.typeName) : 0;
  ...
  const deleteType = (id) => {
    // dispatch(DeleteType(id)); Here use Dispatch
    setDeleteOpen(!open);
  };
  ...
  return (
    <div>
      ...
      {response.loading ?
        ...
        <>
          {response.noData ?
            <DataNotFound title={"No Type Found"} button={"+ Add Type"} />
            :

            <div className="p-4">
              <ListHead
                title={"Type Table"}
                records={response.types.length}
                buttonText={"+ Add Type"}
                searchData={search}
                    ...
                    <tr className=" text-[#101418] capitalize leading-normal w-[100%] ">
                      ...
                      <th className="py-[1%] w-[20%] text-[.8rem] text-gray-700   text-left">
                        ...
                        <Tooltip title={`Change Order`} placement="bottom">
                          <span onClick={() => toggleSortOrder('type')} className="ml-2 cursor-pointer">
                    ...
                    <tbody key={index} className="text-gray-600 text-sm font-light w-[100%] border-t-[1px] border-gray-200">
                      <tr className={` bg-white  `}>
                        ...
                        <td className=" w-[20%] max-md:text-[.7rem] text-left">
                          <p className="font-[600] text-gray-600 text-[14px]">{value.typeName}</p>
                        </td>
