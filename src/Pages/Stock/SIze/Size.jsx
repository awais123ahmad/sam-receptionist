...
import { fetchSizes } from '../../../Api/Reducers/StockReducers/size'
...
const Size = () => {
  ...
  const response = useSelector((state) => state.size);
  ...
  useEffect(() => {
    setInitialData(response.sizes)
  }, [response.sizes])

  useEffect(() => {
    setFilteredData(initialData.filter(size => size.sizeName?.toLowerCase().includes(search.toLowerCase())));
  }, [search, initialData]);
  ...
  const sortedData = [...filteredData]?.sort((a, b) => {
    if (sortField === "size") {
      return sortOrder === "asc" ? a.sizeName.localeCompare(b.sizeName) : sortOrder === "desc" ? b.sizeName.localeCompare(a.sizeName) : 0;
  ...
  const deleteSize = (id) => {
    // dispatch(DeleteSize(id)); Here use Dispatch
    setDeleteOpen(!open);
  };
  ...
  return (
    <div>
      ...
      {response.loading ?
        ...
        <>
          {response.noData ?
            <DataNotFound title={"No Size Found"} button={"+ Add Size"} />
            :

            <div className="p-4">
              <ListHead
                title={"Size Table"}
                records={response.sizes.length}
                buttonText={"+ Add Size"}
                searchData={search}
                    ...
                    <tr className=" text-[#101418] capitalize leading-normal w-[100%] ">
                      ...
                      <th className="py-[1%] w-[20%] text-[.8rem] text-gray-700 text-left">
                        ...
                        <Tooltip title={`Change Order`} placement="bottom">
                          <span onClick={() => toggleSortOrder('size')} className="ml-2 cursor-pointer">
                    ...
                    <tbody key={index} className="text-gray-600 text-sm font-light w-[100%] border-t-[1px] border-gray-200">
                      <tr className={` bg-white  `}>
                        ...
                        <td className=" w-[20%] max-md:text-[.7rem] text-left">
                          <p className="font-[600] text-gray-600 text-[14px]">{value.sizeName}</p>
                        </td>
