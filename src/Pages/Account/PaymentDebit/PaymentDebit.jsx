...
import { Button, Pagination, Stack, ThemeProvider, Tooltip, createTheme } from "@mui/material";
import SearchIcon from "@mui/icons-material/Search";
import { Link } from "react-router-dom";
import PaginationComponent from "../../../Components/PaginationComponent";
import AddPaymentDebit from "./AddPaymentDebit";
import { useDispatch, useSelector } from "react-redux";
import NoData from "../../../Components/NoData";
import { fetchAllDebit } from "../../../Api/Reducers/AccountReducers/debit";
...
const PaymentDebit = () => {
  ...
  const loading = useSelector((state) => state.debit.loading);
  const nodata = useSelector((state) => state.debit.noData);
  ...
  useEffect(() => {
    if (debit) {
      const filteredResult = debit?.filter(
        (item) =>
          (item._id &&
            item._id
              .toLowerCase()
  ...
  return (
    <div>
      ...
      <div className="p-4">
        <h1 className="ml-[3%] text-[19px] text-gray-700 font-[700]">Payment Debit Table</h1>
              ...
              <tbody className="text-gray-600 text-sm font-light w-[100%] border-t-[1px] border-gray-200">
                <tr className={` bg-white  `}>
                  ...
                  <td className=" w-[20%] max-md:text-[.7rem] text-left">
                    <p className="font-[600] text-gray-600 text-[14px]">Lateef</p>
